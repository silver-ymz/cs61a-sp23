...
def perfect_square(score):
    """Judge if the score is a square number"""
    return sqrt(score) == floor(sqrt(score))


def next_perfect_square(score):
    """Return the next perfect square number"""
    return (floor(sqrt(score)) + 1) ** 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    return all(
        strategy(0, 0) == strategy(x, y) for x in range(0, goal) for y in range(0, goal)
    )
    # END PROBLEM 7
...
def make_averaged(original_function, total_samples=1000):
    ...
    # BEGIN PROBLEM 8
    def func(*args):
        sum_numbers = sum(original_function(*args) for _ in range(total_samples))
        return sum_numbers / total_samples
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    avg_dice = make_averaged(lambda n: roll_dice(n, dice), total_samples)
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    res_score = score + tail_points(opponent_score)
    if perfect_square(res_score):
        res_score = next_perfect_square(res_score)
    if res_score - score >= threshold:
        return 0
    return num_rolls
    # END PROBLEM 11


num_rolls_avg_score = None
...
def final_strategy(score, opponent_score, num_rolls=6):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    global num_rolls_avg_score
    if num_rolls_avg_score is None:
        num_rolls_avg_score = make_averaged(lambda: roll_dice(num_rolls))()
    res_score = score + tail_points(opponent_score)
    if perfect_square(res_score):
        res_score = next_perfect_square(res_score)
    if res_score >= 100 or res_score - score >= num_rolls_avg_score:
        return 0
    return num_rolls
    # END PROBLEM 12
