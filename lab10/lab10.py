...
def subseqs(s):
    ...
    if s == []:
        ...
        a = subseqs(s[1:])
        return a + insert_into_all(s[0], a)
...
def partition_gen(n):
    ...
    def yield_helper(j, k):
        if k == 0:
            ...
            for small_part in yield_helper(j, k - j):
                yield [j] + small_part
            yield from yield_helper(j - 1, k)
...
def reverse_other(t: "Tree"):
    ...
    def helper(t: "Tree", rev=True):
        b = t.branches
        if rev:
            for i in range(len(b) // 2):
                b[i].label, b[::-1][i].label = b[::-1][i].label, b[i].label
        for t in b:
            helper(t, not rev)
