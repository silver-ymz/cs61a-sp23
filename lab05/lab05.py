...
def flatten(s):
    ...
    [[1, [1, [1, [1, 1, [1, 1, [1]]]], 1]]]
    """
    return [s] if type(s) != list else reduce(add, map(flatten, s), [])
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    city = min(city_a, city_b, key=lambda city: distance(city, make_city("", lat, lon)))
...
def count_palindromes(L):
    ...
    2
    """
    return sum(1 for i in L if (s := i.lower()) == s[::-1])
...
def coords(fn, seq, lower, upper):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [[x, y] for x in seq if (y := fn(x)) >= lower and y <= upper]
