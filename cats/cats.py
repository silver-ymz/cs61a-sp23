...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    return lambda str: any(
        remove_punctuation(s.lower()) in subject for s in str.split()
    )
    # END PROBLEM 2
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    [value, index] = min(
        (diff_function(typed_word, v, limit), k) for k, v in enumerate(word_list)
    )
    if value > limit:
        return typed_word
...
def final_diff(typed, source, limit):
    ...
    if typed == "" or source == "":
        ...
        if typed[0] == source[0]:
            substitute = final_diff(typed[1:], source[1:], limit)
        else:
            substitute = 1 + final_diff(typed[1:], source[1:], limit - 1)
        if len(typed) < 2 or len(source) < 2:
            swap = limit + 1
        elif typed[0] == source[1] and typed[1] == source[0]:
...
def report_progress(typed, prompt, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    progress = sum(
        1 for _ in takewhile(lambda x: x[0] == x[1], zip(typed, prompt))
    ) / len(prompt)
...
def time_per_word(words, times_per_player):
    ...
    return match(
        words,
        [
            [after - prev for prev, after in zip(times, times[1:])]
...
def fastest_words(match):
    ...
    for i in word_indices:
        min_player = min(player_indices, key=lambda k: get_all_times(match)[k][i])
        result[min_player] += [get_all_words(match)[i]]
    return result
    # END PROBLEM 10
